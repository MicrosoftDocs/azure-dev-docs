...
[Azure Skills](../azure-skills/overview.md) - including `azure-prepare`, `azure-validate`, and `azure-deploy` - are also available to enable structured multistep workflows for common Azure tasks. In extension-based environments such as Visual Studio Code, they install automatically with GitHub Copilot for Azure. In Visual Studio 2022 and Visual Studio 2026, Azure and related .NET skills and tools surface through installed workloads and appear in the Copilot tools and skills picker, where they're off by default until you enable them.
...
|Visual Studio 2022|Surfaces built-in Azure MCP Server tools via the GitHub Copilot user interface. Available in Visual Studio 2022 v17.14.30+; no separate GitHub Copilot for Azure extension is required. Tools are installed with the Azure development workload and receive updates via regular Visual Studio updates (Visual Studio Installer). When the corresponding Azure and .NET workloads are installed, Azure and related .NET skills/tools appear in the Copilot tools/skills picker. These skills/tools are off by default until you enable them, and enabled selections persist across sessions. Requires a GitHub Copilot subscription and Azure sign-in. Tool availability reflects Azure subscription permissions.|Public preview|The tools are available upon installation of the [Azure development workload](/dotnet/azure/configure-visual-studio#install-azure-workloads).|
|Visual Studio 2026|Surfaces built-in GitHub Copilot for Azure and Azure MCP Server tools via the GitHub Copilot user interface. With the Azure and AI development workload (and related .NET workloads), Azure and related .NET skills/tools appear in the Copilot tools/skills picker. These skills/tools are off by default until you enable them.|General availability|The tools are available upon installation of the [Azure and AI development workload](/dotnet/azure/configure-visual-studio#install-azure-workloads).|
...
Here's a list of all the tools currently supported by GitHub Copilot for Azure. Also listed is the migration status, which applies solely to the Visual Studio Code version. Visual Studio 2022 and Visual Studio 2026 use built-in Azure MCP Server tools.
...
GitHub Copilot for Azure uses agentic tools supplied by the [Azure MCP Server](../azure-mcp-server/overview.md) behind the scenes to perform all operations. The tools and skills appear in the Copilot tools and skills picker and are off by default until you enable them.

# [Visual Studio 2026](#tab/vs2026)

GitHub Copilot for Azure uses agentic tools supplied by the [Azure MCP Server](../azure-mcp-server/overview.md) behind the scenes to perform all operations. The tools and skills appear in the Copilot tools and skills picker and are off by default until you enable them. In addition to Azure MCP Server tools, the following Visual Studio-specific tools are also available.
