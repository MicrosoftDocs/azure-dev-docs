...
- If the `az` command isn't recognized, be sure you have the Azure CLI installed as described in [Set up your initial environment](../../quickstart-python-flask-multipage.yml?tutorial-step=1).
- If the `webapp` command isn't recognized, be sure that your Azure CLI version is 2.0.80 or higher. If it isn't, [install the latest version](/cli/azure/install-azure-cli).
...
The command may take a few minutes to complete. While running, it provides messages about creating the resource group, the App Service plan and hosting app, configuring logging, and performing ZIP deployment. It then gives the message, "You can launch the app at http://&lt;app-name&gt;.azurewebsites.net", which is the app's URL on Azure.
