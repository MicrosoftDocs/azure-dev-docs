...
- Run [terraform plan](https://www.terraform.io/docs/commands/validate.html) to ensure the configuration  works as expected.
...
Unit tests ensure a specific part or function of a program behaves correctly. The developer of the functionality writes unit tests. Sometimes called test-driven development, or TDD, this type of testing involves continuous short development cycles. In the context of Terraform projects, unit testing can take the form of using `terraform plan` to ensure that the actual values available in the generated plan equal the expected values.
...
Use compliance testing to ensure the configuration follows the policies you define for the project. For example, you might define geopolitical naming conventions for your Azure resources. Or you might want virtual machines to be created from a defined subset of images. Use compliance testing to enforce these rules.

Typically, define compliance testing as part of the continuous integration process.
...
E2E testing is typically a three-step process. First, apply the configuration to a test environment. Then, run code to verify the results. Finally, reinitialize or take down the test environment (such as deallocating a virtual machine).

> [!div class="nextstepaction"]
> [Learn more about end-to-end Testing](best-practices-end-to-end-testing.md)

> [!div class="nextstepaction"]
> [Prevent Terraform drift for stateful Azure resources](prevent-terraform-drift-stateful-resources.md)
