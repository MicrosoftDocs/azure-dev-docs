...
This article provides an overview of how to enable logging in applications that use the Azure SDK for Java. The Azure client libraries for Java offer two logging options:

* A built-in logging framework for temporary debugging purposes.
* Support for logging by using the [SLF4J](https://www.slf4j.org/) interface.

Use SLF4J because it's well known in the Java ecosystem and it's well documented. For more information, see the [SLF4J user manual](https://www.slf4j.org/manual.html).
...
Whatever logging configuration you use, you get the same log output because all logging output in the Azure client libraries for Java routes through an azure-core `ClientLogger` abstraction.
...
Use the `AZURE_HTTP_LOG_DETAIL_LEVEL` environment variable to turn on HTTP logs for all clients. Set this variable to one of the following values:

- `NONE`: HTTP logs are off. This value is the default.
- `BASIC`: HTTP logs include the request method, sanitized request URL, try count, response code, and the content length for request and response bodies.
...
> [!NOTE]
> The request URL is sanitized - that is, all query parameter values are redacted except for the `api-version` value. Individual client libraries might add other query parameters that are known to be safe to the allowlist.
...
> [!WARNING]
> Don't log request and response bodies in production because they might contain sensitive information, significantly affect performance, change how content is buffered, and have other side effects.
...
This code enables HTTP logs with headers and adds the `Accept-Ranges` response header and the `label` query parameter to the corresponding allowlists. After this change, these values appear in the produced logs.
...
As noted, all Azure client libraries use SLF4J for logging, but there's a fallback, default logger built into Azure client libraries for Java. This default logger is for cases where you deploy an application and need logging, but can't redeploy the application with an SLF4J logger included. To enable this logger, first ensure that no SLF4J logger exists (because it takes precedence), and then set the `AZURE_LOG_LEVEL` environment variable. The following table shows the values allowed for this environment variable:
...
After you set the environment variable, restart the application to enable the environment variable. This logger logs to the console, and doesn't provide the advanced customization capabilities of an SLF4J implementation, such as rollover and logging to file. To turn the logging off, remove the environment variable and restart the application.

## SLF4J logging

By default, configure logging by using an SLF4J-supported logging framework. First, include a relevant SLF4J logging implementation as a dependency from your project. For more information, see [Declaring project dependencies for logging](http://www.slf4j.org/manual.html#projectDep) in the SLF4J user manual. Next, configure your logger to work as necessary in your environment, such as setting log levels, configuring which classes do and don't log, and so on. Some examples are provided through the links in this article, but for more information, see the documentation for your chosen logging framework.

## Log format

Logging frameworks support custom log message formatting and layouts. Include at least the following fields to make it possible to troubleshoot Azure client libraries:
...
> [!NOTE]
> Azure client library logs are intended for ad-hoc debugging. Don't rely on the log format to alert or monitor your application. Azure client libraries don't guarantee the stability of log messages or context keys. For such purposes, use distributed tracing. The Application Insights Java agent provides stability guarantees for request and dependency telemetry. For more information, see [Configure tracing in the Azure SDK for Java](tracing.md).
