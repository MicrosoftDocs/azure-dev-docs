---
title: Authenticate Java apps during local development using service principals
titleSuffix: Azure SDK for Java
description: Learn how to authenticate Java apps to Azure services during local development using application service principals.
ms.date: 01/30/2026
ms.topic: how-to
...
# Authenticate Java apps to Azure services during local development using service principals
...
Check the returned credentials and note down the following information:
...
Note down the path/location of the certificate for use with `ClientCertificateCredential`.
...
At runtime, certain credentials from the [Azure Identity library](/java/api/com.azure.identity), such as `DefaultAzureCredential`, `EnvironmentCredential`, and `ClientSecretCredential`, search for service principal information by convention in the environment variables. There are multiple ways to configure environment variables when working with Java, depending on your tooling and environment.
...
Add the following lines to your `~/.bashrc` or `~/.zshrc` file, replacing the placeholder values with the actual values from your app registration:
...
#### [VS Code](#tab/vscode-env)

In VS Code, you can set environment variables using a `.env` file in your project or through the `launch.json` configuration. Create a `.env` file in your project root:
...
Azure services are accessed using specialized client classes from the Azure SDK client libraries. The following code samples demonstrate how to configure credentials for service principal authentication.

#### Use DefaultAzureCredential

`DefaultAzureCredential` is the recommended credential for most scenarios because it automatically discovers credentials from environment variables (when configured for a service principal) and falls back to other credentials in development environments.
