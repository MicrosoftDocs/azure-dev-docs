...
| 4     | [Shared Token Cache][stc-cred]       |If the developer authenticated to Azure by logging into Visual Studio, authenticate the app to Azure using that same account. (Windows only.)           | Yes                 |
| 5     | [IntelliJ][ij-cred]         |If the developer authenticated via Azure Toolkit for IntelliJ, authenticate that account.             | Yes                  |
