...
You can manually verify that you've properly authenticated the Azure CLI and can obtain tokens. First, use the `account` command to verify that the account is currently signed in to the Azure CLI.
...
When you use `VisualStudioCodeCredential`, you may optionally try/catch for `CredentialUnavailableException`. The following table shows the errors that this exception indicates, and methods of mitigation:
...
| `MSAL Interaction Required Exception`                                                            | The `VisualStudioCodeCredential` was able to read the cached credentials from the cache but the cached token is likely expired. | Sign in to the Azure Account extension via **View > Command Palette** to execute the **Azure: Sign In** command in the VS Code IDE.                                                                                                                                                                                                                                                                                                                                                                                                                           |
