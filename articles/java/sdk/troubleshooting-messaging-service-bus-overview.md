...
The Service Bus client supports renewing the message lock continuously each time before it expires. For this, the client will run a background lock renew task. By default, the lock renew task will run for a duration of 5 minutes, this duration can be adjusted using `ServiceBusReceiverClientBuilder::maxAutoLockRenewDuration(Duration)` API or can be turned off by setting it `Duration#ZERO`.
...
* The host system time isn't accurate, for example, the clock is skewed, preventing lock renewal to happen on time.
...
* The previously mentioned I/O Thread bottleneck and resulting lock lost can also happen if application has set a high `maxConcurrentCalls` value in `ServiceBusProcessorClient` or a `maxMessages` in `ServiceBusReceiverClient::receiveMessages`. See the section [Concurrency in ServiceBusProcessorClient](#concurrency-in-servicebusprocessorclient).

Running many lock renewal tasks in the client (as a result of high ` maxConcurrentCalls` or `maxMessages`) may put more stress on low CPU environments, see [Containerize your Java applications](../containers/overview.md). Given each lock renewal is a network call to the Service Bus broker, a high number of lock renewal tasks making multiple lock renew network calls also may have an adverse effect in Service Bus namespace throttling.

The same observations related to locks apply for a session enabled Service Bus queue or subscription in a topic as well. When the receiver client connects to a session in the resource, the broker will apply an initial lock to the session. To maintain the lock on the session, the lock renew task in the client has to keep renewing the session lock before it expires. For a session enabled resource, the underlying partitions sometimes move to achieve load balancing across Service Bus nodes; for example, when new nodes are added to share the load. When that happens, session locks can be lost. If the application tries to complete or abandon a message after the session lock is lost, the API call will fail with the error saying `com.azure.messaging.servicebus.ServiceBusException: The session lock was lost. Request a new session receiver`. 
