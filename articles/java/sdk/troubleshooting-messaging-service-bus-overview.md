...
* The application has turned on the Prefetch feature by setting the prefetch count to a positive number using `ServiceBusReceiverClientBuilder::prefetchCount(prefetch)`. When the Prefetch feature is enabled, the client will retrieve the number of messages equal to the prefetch from the Service Bus entity - queue or topic - and store them in the in-memory prefetch buffer. The messages stay in the prefetch buffer until they are received into the application. The client will not extend the lock of the messages while they are in the prefetch buffer, this means these messages will have only the initial lock that Service Bus broker applied. If the application processing takes so long that message locks expire while staying in the prefetch buffer, then the application might acquire the messages with an expired lock. For more information, see [Why is Prefetch not the default option?](/azure/service-bus-messaging/service-bus-prefetch?tabs=java#why-is-prefetch-not-the-default-option)
