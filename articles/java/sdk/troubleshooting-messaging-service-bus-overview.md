---
title: Troubleshoot Azure Service Bus issues
titleSuffix: Azure SDK for Java
description: Learn how to troubleshoot Azure Service Bus issues in the Azure SDK for Java. Diagnose errors, configure logging, and resolve common failures.
ms.date: 02/23/2024
...
This article covers failure investigation techniques, concurrency, and common errors in the Azure Service Bus Java client library. Use this guidance to identify root causes and apply mitigation steps to resolve Azure Service Bus issues faster.
...
1. Add the dependencies in your **pom.xml** by using ones from the [logging sample pom.xml][LoggingPom], in the "Dependencies required for Log4j2" section.
...
1. Add the dependencies in your **pom.xml** by using ones from the [logging sample pom.xml][LoggingPom], in the "Dependencies required for logback" section.
...
The custom endpoint address refers to an application-provided HTTPS endpoint address that you can resolve to Service Bus or configure to route traffic to Service Bus. Azure Application Gateway makes it easy to create an HTTPS front end that forwards traffic to Service Bus. You can configure the Service Bus SDK for an application to use an Application Gateway front-end IP address as the custom endpoint to connect to Service Bus.

Application Gateway offers several security policies that support different TLS protocol versions. There are predefined policies that enforce TLS 1.2 as the minimum version, and there are older policies that use TLS 1.0 as the minimum version. You apply a TLS policy to the HTTPS front end.

Currently, the Service Bus SDK doesn't recognize certain remote TCP terminations by the Application Gateway front end, which uses TLS 1.0 as the minimum version. For example, if the front end sends TCP FIN and ACK packets to close the connection when it updates its properties, the SDK can't detect these packets. So, it doesn't reconnect, and clients can't send or receive messages anymore. This halt only happens when using TLS 1.0 as the minimum version. To mitigate this problem, use a security policy that sets TLS 1.2 or higher as the minimum version for the Application Gateway front end.

Support for TLS 1.0 and 1.1 across all Azure services is already [announced](https://azure.microsoft.com/updates/azure-support-tls-will-end-by-31-october-2024-2) to end by 31 October 2024, so transition to TLS 1.2.
...
The following list describes some of the usage patterns or host environments that can lead to the lock lost error:
...
* The application's message processing time exceeds the configured lock renew task duration. Note that, if the lock renew duration isn't set explicitly, it defaults to 5 minutes.
* The application turns on the Prefetch feature by setting the prefetch value to a positive integer using `ServiceBusReceiverClientBuilder.prefetchCount(prefetch)`. When the Prefetch feature is enabled, the client retrieves the number of messages equal to the prefetch from the Service Bus entity - queue or topic - and stores them in the in-memory prefetch buffer. The messages stay in the prefetch buffer until they're received into the application. The client doesn't extend the lock of the messages while they're in the prefetch buffer. If the application processing takes so long that message locks expire while staying in the prefetch buffer, the application might acquire the messages with an expired lock. For more information, see [Why is Prefetch not the default option?](/azure/service-bus-messaging/service-bus-prefetch?tabs=java#why-is-prefetch-not-the-default-option)
...
* The connection I/O thread is overloaded, impacting its ability to execute lock renew network calls on time. The following two scenarios can cause this issue:
  ...
  * The application configures `ServiceBusReceiverClient.receiveMessages` or `ServiceBusProcessorClient` to have large `maxMessages` or `maxConcurrentCalls` values. For more information, see the [Concurrency in ServiceBusProcessorClient](#concurrency-in-servicebusprocessorclient) section.
...
If the host isn't sufficiently resourced, the lock can still be lost even if there are only a few lock renew tasks running. If you're running the Java application on a container, use two or more vCPU cores. Don't select anything less than 1 vCPU core when running Java applications on containerized environments. For in-depth recommendations on resourcing, see [Containerize your Java applications](../containers/overview.md).
