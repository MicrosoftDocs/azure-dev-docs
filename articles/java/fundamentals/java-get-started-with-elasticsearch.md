...
If you are already familiar with deploying an Elasticsearch cluster, this tutorial still covers very useful information to even more experienced users. It identifies and articulates various benefits applicable for Java developers. At various times, details around the various out-of-the-box solutions are discussed, such as the Elastic APM server and APM Java Agent, and how Kibana can provide insight into the environment both during development and at run-time..
...
This is possible Kibana brings together the relevant logs and metrics, into one centralized location. The various APM agents capture [instrumented application events](https://www.elastic.co/guide/en/apm/get-started/7.8/apm-data-model.html) such as spans, transactions, errors and metrics.
...
Additionally, you may want to enable modules relevant to your environment, such as if you are running Nginx or MySQL. There is also a **Systems Logs module** you can enable upon installing Filebeat. Check out all the out-of-the-box types of [Elastic Integrations](https://www.elastic.co/integrations). Many come with pre-built dashboards and visualizations, getting you up and running even faster!  
...
The procedure differs between Unix/Linux and Windows systems. Windows has its own **Windows metrics** module and a separate installation procedure for Filebeat. For Unix/Linux systems, you can install the Filebeat along with installing the System logs in the same procedure.

#### Filebeat and System logs for Unix/Linux

Click **Add log data** from the Kibana main page.
...
> [!IMPORTANT]
> During the second step, we will want to make one additional configuration change to the *filebeat.yml* file in order to also collect the relevant log files you wish to import into Kibana.

In order to make the connection between the application server and Elasticsearch, you must set two variables, the `cloud.id`, which has been provided, and the `cloud.auth`, requiring you to just modify the &lt;password&gt;.
...
#### Filebeat and Windows Metrics for Windows systems

Since Filebeat is a separate installation procedure than the Windows Metrics, we will refer to the official [Getting Started with Filebeat](https://www.elastic.co/guide/en/beats/filebeat/7.8/filebeat-installation.html) reference guide for the proper procedure on installing and configuring Filebeat.  

In order to install Windows Metrics, you will follow the procedure depicted in Kibana.

Click **Add metric data** from the Kibana main page.

Choose **Windows Metrics**.

Ensure to select the **Windows** procedure tab and follow the instructions.  

## Your window into the environment

Now that you have configured the APM Java Agent and Filebeat, configured to ingest logs as well as enabled at least one module, we can begin to take advantage of our hard work by searching and visualizing that data ingested.

### Viewing the Filebeat System dashboard

Feel free to look around at the various system logs and metrics collected. We will move on to focus on the APM Java Agent, as that has been our main focus of this tutorial.

Please do notice the various visualizations that come out-of-the-box in dashboards for Syslog, Sudo commands, SSH logins and New users and groups. To visualize from the Kibana main page, click on *Dashboards** and search for *sys*. Try it out by running a few sudo commands or connecting with SSH including throwing in a bad credential, looking at their respective dashboards.
...
### Viewing APM Services
From the navigation menu, by clicking the uppermost left menu button, click **APM** under Observability. Additionally, from the main page simply click APM.

Click the service name you had recently configured. The Agent column should say **java**.

You should see Java relevant information such as translation information, i.e. time based details per resource type including the actual transaction names.

There are also tabs for depicting Errors, JVM performance metrics, and a service map.

Make sure you adjust the time range in the upper right side of the screen.  
...
Use Filebeat for logs:
Prerequisites (for Filebeat) - logs need to be in Azure Event Hub (https://www.elastic.co/guide/en/beats/filebeat/7.8/filebeat-module-azure.html)

Use Metricbeat for Azure Monitor metrics: https://www.elastic.co/guide/en/beats/metricbeat/7.8/metricbeat-module-azure.html

Both comes with OOTB dashboards
