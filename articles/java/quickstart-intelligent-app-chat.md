...
1. In the **Settings** tab, deselect **Use semantic ranker for retrieval**. Semantic ranker is a feature of [Azure Cognitive Search](/azure/search/semantic-search-overview#what-is-semantic-search) that uses machine learning to improve the relevance of search results.
...
The backend `/chat` API steps through the process of getting the intelligent answer:
* Build RAG options: Create a set of options that will be used to generate an intelligent answer.
