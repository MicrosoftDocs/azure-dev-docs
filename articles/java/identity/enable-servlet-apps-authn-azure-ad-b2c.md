---
title: Java Servlet Web App using MSAL4J to authenticate users into Azure Active Directory B2C
...
# Java Servlet Web App using MSAL4J to authenticate users into Azure Active B2C

This article shows you how to authenticate your Java Servlet web app to Azure Active Directory B2C by using the MSAL client library for Java. Once authenticated, your target user ID (principal) will have the runtime security credentials to be fully authenticated (AuthN) against Azure Active Directory B2C.  

The [sample code snippets](https://github.com/Azure-Samples/ms-identity-msal-java-samples/tree/main/3.%20Java%20Servlet%20Web%20App%20Tutorial/1-Authentication/sign-in-b2c) are available in GitHub as runnable Java files.
...
aad.signInPolicy=b2c_1_susi/
aad.passwordResetPolicy=b2c_1_reset/
aad.editProfilePolicy=b2c_1_edit_profile/

aad.signOutEndpoint=oauth2/v2.0/logout/
aad.postSignOutFragment=?post_logout_redirect_uri=

aad.forgotPasswordErrCode=AADB2C90118
aad.errorDescriptionKey=error_description

app.redirectUri=http://localhost:8080/ms-identity-b2c-java-servlet-webapp-authentication/auth_redirect
#  in seconds:
app.stateTTL=600
...
1. Open the [authentication.properties]([src/main/resources/authentication.properties](https://github.com/Azure-Samples/ms-identity-msal-java-samples/tree/main/3.%20Java%20Servlet%20Web%20App%20Tutorial/1-Authentication/sign-in-b2c/src/main/resources)) file.
...
1. The client Java Servlet web application uses **MSAL4J** to sign-in users and obtains an [ID Token](/azure/active-directory/develop/id-tokens) from **Azure AD B2C**:
2. The **ID Token** proves that the user has successfully authenticated against a **Azure AD B2C** tenant.

![Scenario](./media/identity-authentication-servlets/identity15.png)
...
![Experience](./media/identity-authentication-servlets/identity16.png)
...
> :information_source: Did the sample not work for you as expected? Then please reach out to us using the [GitHub Issues](https://github.com/Azure-Samples/ms-identity-msal-java-samples/issues) page.

## About the code

This sample demonstrates how to use **MSAL4J** to sign in users into your Azure AD B2C tenant.

A **ConfidentialClientApplication** instance is created in the [AuthHelper.java](https://github.com/Azure-Samples/ms-identity-msal-java-samples/tree/main/3.%20Java%20Servlet%20Web%20App%20Tutorial/1-Authentication/sign-in-b2c/src/main/java/com/microsoft/azuresamples/authenticationb2c/AuthHelper.java) class. This object helps craft the AAD B2C authorization URL and also helps exchange the authentication token for an access token.
...
In this sample, these values are read from the [authentication.properties](https://github.com/Azure-Samples/ms-identity-msal-java-samples/tree/main/3.%20Java%20Servlet%20Web%20App%20Tutorial/1-Authentication/sign-in-b2c/src/main/resources/authentication.properties) file using a properties reader in the class [Config.java](https://github.com/Azure-Samples/ms-identity-msal-java-samples/tree/main/3.%20Java%20Servlet%20Web%20App%20Tutorial/1-Authentication/sign-in-b2c/src/main/java/com/microsoft/azuresamples/authenticationb2c/Config.java).
...
1. The first step of the sign-in process is to send a request to the `/authorize` endpoint on for our Azure Active Directory B2C Tenant. Our MSAL4J ConfidentialClientApplication instance is leveraged to construct an authorization request URL, and our app redirects the browser to this URL.
    ...
    - **SCOPES**: [Scopes](/azure/active-directory-b2c/access-tokens#scopes) are permissions requested by the application.
      ...
      - Full list of scopes requested by the app can be found in the [authentication.properties file](https://github.com/Azure-Samples/ms-identity-msal-java-samples/tree/main/3.%20Java%20Servlet%20Web%20App%20Tutorial/1-Authentication/sign-in-b2c/src/main/resources/authentication.properties).
    ...
    - **state**: a unique variable set by the app into the session on each token request, and destroyed after receiving the corresponding AAD redirect callback. The state variable ensures that AAD requests to the [/auth_redirect endpoint](https://github.com/Azure-Samples/ms-identity-msal-java-samples/tree/main/3.%20Java%20Servlet%20Web%20App%20Tutorial/1-Authentication/sign-in-b2c/src/main/java/com/microsoft/azuresamples/authenticationb2c/AADRedirectServlet.java) are actually from AAD authorization requests originating from this app and this session, thereby preventing CSRF attacks.
...
1. The user is presented with a sign-in prompt by Azure Active Directory B2C. If the sign-in attempt is successful, the user's browser is redirected to our app's redirect endpoint. A valid request to this endpoint will contain an [**authorization code**](/azure/active-directory-b2c/authorization-code-flow).
...
1. If the nonce is successfully validated, authentication status is put into a server-side session, leveraging methods exposed by the class [MsalAuthSession.java](https://github.com/Azure-Samples/ms-identity-msal-java-samples/tree/main/3.%20Java%20Servlet%20Web%20App%20Tutorial/1-Authentication/sign-in-b2c/src/main/java/com/microsoft/azuresamples/authenticationb2c/MsalAuthSession.java):
