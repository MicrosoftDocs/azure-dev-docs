...
description: This article describes how Spring Cloud Azure and Spring Messaging can be used together.
ms.date: 04/07/2023
author: KarlErickson
ms.author: hangwan
ms.topic: reference
...
The Spring Framework provides extensive support for integrating with messaging systems, Spring Messaging for Azure support these Azure services: Event Hubs, Service Bus, Storage Queue. The following is a list of supported Spring Messaging Azure services:

* `spring-messaging-azure-eventhubs` - for more information, see [Spring Messaging Azure Event Hubs](#spring-messaging-azure-event-hubs)
* `spring-messaging-azure-servicebus` - for more information, see [Spring Messaging Azure Service Bus](#spring-messaging-azure-service-bus)
* `spring-messaging-azure-storage-queue` - for more information, see [Spring Messaging Azure Storage Queue](#spring-messaging-azure-storage-queue)

## Spring Messaging Azure Event Hubs

### Key concepts

Azure Event Hubs is a native data-streaming service in the cloud that can stream millions of events per second, with low latency, from any source to any destination. Spring Messaging for Azure Event Hubs project applies core Spring concepts to the development of event hubs-based messaging solutions. It provides a "template" as a high-level abstraction for sending messages. It also provides support for Message-driven POJOs with `@EventHubsListener` annotations and a "listener container". These libraries promote the use of dependency injection and declarative. In all of these cases, you will see similarities to the JMS support in the Spring Framework and RabbitMQ support in Spring AMQP.
...
Providing the following configuration options to use `EventHubsTemplate` and `@EventHubsListener`:
...
1. `EventHubsTemplate` is auto-configured, and you can autowire it directly into your own beans, as shown in the following example:
...
1. Add the `@EnableAzureMessaging` annotation, which triggers the discovery of methods annotated with `@EventHubsListener`, creating the message listener container under the covers.
...
1. When the EventHubs infrastructure is present, any bean can be annotated with `@EventHubsListener` to create a listener endpoint. The following component creates a listener endpoint on the `EVENT_HUB_NAME` event hub and `$Default` Consumer group:
...
Providing the following configuration options to use `ServiceBusTemplate` and `@ServiceBusListener`:
...
1. `ServiceBusTemplate` is auto-configured, and you can autowire it directly into your own beans, as shown in the following example:
...
1. When the ServiceBus infrastructure is present, any bean can be annotated with `@ServiceBusListener` to create a listener endpoint. The following component creates a listener endpoint on the `QUEUE_NAME` queue:
...
Azure Queue Storage is a service for storing large numbers of messages. You access messages from anywhere in the world via authenticated calls using HTTP or HTTPS. A queue message can be up to 64 KB in size. A queue may contain millions of messages, up to the total capacity limit of a storage account. Queues are commonly used to create a backlog of work to process asynchronously. Spring Messaging for Azure Queue Storage project applies core Spring concepts to the development of service bus-based messaging solutions. It provides a "template" as a high-level abstraction for sending and receiving messages. These libraries promote the use of dependency injection and declarative.
...
Providing the following configuration options to use `StorageQueueTemplate`:
...
1. `StorageQueueTemplate` is auto-configured, you can autowire it directly into your own beans and send/receive messages, as shown in the following example:
