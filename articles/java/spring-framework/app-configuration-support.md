...
The Azure Spring Cloud App Configuration Client Library loads configurations and feature flags from the [Azure App Configuration service](~/azure/azure-app-configuration/). The client library generates `PropertySource` abstractions, to match those already being generated by the Spring environment such as; environment variables, command-line configurations, local configuration files, and so on.
