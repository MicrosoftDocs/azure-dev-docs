...
The Azure Spring Cloud App Configuration library loads configurations and feature flags from the [Azure App Configuration service](/azure/azure-app-configuration/index). The library generates `PropertySource` abstractions, to match those already being generated by the Spring environment such as; environment variables, command-line configurations, local configuration files, and so on.
...
[Spring Cloud Azure configuration](/azure/developer/java/spring-framework/configuration) can be used to configure the library. The following properties can be used to configure the library:
...
Azure App Configuration and its libraries support referencing secrets stored in Key Vault. In App Configuration, keys can be created with values that map to a secret stored in a Key Vault. Secrets are securely stored in Key Vault, but can be accessed in the same way as any other configuration once loaded.
...
[Spring Cloud Azure configuration](/azure/developer/java/spring-framework/configuration) can be used to configure the library. The same credential used to connect to App Configuration can be used to connect to Azure Key Vault.
