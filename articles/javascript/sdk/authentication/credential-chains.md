...
To ensure the environment variable is defined and set to a supported string, set the `requiredEnvVars` property in [DefaultAzureCredentialOptions](/javascript/api/@azure/identity/defaultazurecredentialoptions#@azure-identity-defaultazurecredentialoptions-requiredenvvars) to `AZURE_TOKEN_CREDENTIALS`:
