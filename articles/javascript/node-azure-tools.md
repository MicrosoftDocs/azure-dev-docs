...
| Azure DocumentDB | [DocumentDB for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-documentdb) <br><br> [Azure CLI](/cli/azure/documentdb) | Browse, manage, and query DocumentDB and MongoDB databases directly from VS Code. DocumentDB is a fully managed, MongoDB-compatible service based on the open-source DocumentDB engine. Supports hybrid and multicloud deployments with predictable pricing. |
