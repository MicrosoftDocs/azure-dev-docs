...
|[Azure SDK: Use Azure Monitor SDK to list recent resource operations](how-to/with-azure-sdk/create-manage-virtual-machine.md)|An Azure Virtual machine requires several resources to support the virtual machine. The best way to manage those resources is to create all the resources in a single resource group. The script creates the resource group and postpends a random number to make sure the resource group is unique, regardless of how many times you use the script.|
