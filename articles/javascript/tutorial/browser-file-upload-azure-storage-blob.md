...
Use a static web app to upload a file to an Azure Storage blob using an Azure Storage [@azure/storage-blob](https://www.npmjs.com/package/@azure/storage-blob) npm package with an Azure Storage SAS token. The SAS token generated by the API is an example of the [Valet Key pattern](https://learn.microsoft.com/en-us/azure/architecture/patterns/valet-key). This design pattern allows you to delegate limited access to secure resources, enhancing security without exposing full credentials.
...
Create a container which has publicly readable blobs. The container isn't publicly accessible. 
