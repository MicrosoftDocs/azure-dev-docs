...
* Query a restricted API for user - such as Graph
...
1. Select **New registration** to begin the app registration process.
    ...
    :::image type="content" source="../../media/express-app-msal-auth/azure-portal-active-directory-app-registrations-new-registration.png" alt-text="Select **New registration** to begin the app registration process.:::
...
1. In the next form, configure the following identity provider settings:
    ...
    |Web Redirect URI|Enter your 2 versions (cloud and local) for the redirect URL:<br>`http://localhost:8080`<br>`https://YOUR-APP-SERVICE-NAME.azurewebsites.net`|
...
1. When the process completes, still in the Azure portal for your new Active Directory app registration, you need to copy the following values to use in your Express.js app:
...
1. Set these three values in the `./appSettings` file for the `credentials` properties. Post the values into the hard-coded string, used only for local development. 
    ...
    The hard-coded strings will be used in local development only for this tutorial. Once you are done with the tutorial, create local environment settings for these values on your development workstation. You set the cloud settings, `process.env.[variable]`, in the next section so your Express.js app will also use the cloud settings. 
...
1. Add the following name/value pairs, each, as a separate application setting.
    ...
    |--|--|
    |BASE_URI|This value should be the name of your app service and is used to build the redirect URL passed in the MSAL SDK.|
...
1. Select **Sign-in** from the top navigation bar.
    ...
    :::image type="content" source="../../media/app-service-easy-authentication/sign-in-to-access-your-resources.png" alt-text="In the web browser for your Express.js app, select **Sign-in** from the top navigation bar.":::
...
1. Review the permissions requested pop-up then select **Accept**. This is a one-time required request for you, and your app's users, to agree to the permissions requested by your Active Directory app.
    ...
    :::image type="content" source="../../media/app-service-easy-authentication/browser-window-permissions-requested-by-active-directory-app.png" alt-text="Review the permissions requested pop-up then select **Accept**. This is a one-time required request for you (your app's users) to agree to the permissions requested by the Active Directory app.":::
...
1. When the sign in process is complete, select **Get my profile**.
    ...
    :::image type="content" source="../../media/app-service-easy-authentication/welcome-get-my-profile.png" alt-text="When the sign in process is complete, select **Get my profile**.":::

1. View your profile data help in the Microsoft Identity provider. The following profile is an example only.

    :::image type="content" source="../../media/app-service-easy-authentication/calling-microsoft-graph.png" alt-text="View your profile data help in the Microsoft Identity provider. The following profile is an example only.":::
...
1. In VS Code, in the Azure side bar, select your App service then right-click and select **Restart**, to have the new app settings take effect.
    ...
    :::image type="content" source="../../media/app-service-easy-authentication/vscode-app-service-restart-app.png" alt-text="In VS Code, in the Azure side bar, select your App service then right-click and select **Restart**.":::
