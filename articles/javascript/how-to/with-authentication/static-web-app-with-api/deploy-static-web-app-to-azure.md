...
In VS Code, review the changes and commit the changes. 

1. In VS Code, select the File menu, then select **Close Folder**. Then select File menu, and **Open Folder** to open the root of the GitHub repo, `ms-identity-javascript-react-tutorial`.
   
   While developing the sample, it was easier to limit the view of the file explorer. Now that you need to create the Static web app, you need to see and understand the entire file structure again. 
...
1. Follow the prompts using the following table to understand how to create your **Azure Static web app** resource.
    ...
    |Enter the location of your Azure Functions code or leave black to skip this step.|Enter `/4-Deployment/2-deploy-static/App/api`.|
...
1. Copy the URL. 
1. In VS Code, in the Azure explorer, select you static web app then select **Open in Portal**. 
...
1. In the *.yml file, dd the React client environment variables needed to use MSAL. Find these values in the `./.env` file for the sample. 
