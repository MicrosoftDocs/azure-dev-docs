...
2. **Plan creation**: Creates an upgrade plan based on your prompt or uses latest LTS versions
3. **Execution**: Applies the upgrade to each repository
4. **Validation**: Builds and validates changes for each repository
...
5. To run the upgrade, select **1. Upgrade locally**.
...
5. To run the upgrade, select **2. Delegate to Cloud Coding Agents**.
...
2. **Review individual repository changes**:
   ```bash
   cd <repository-name>
   git status
   git diff
...
3. **Create pull requests** for successful upgrades:
   ```bash
   cd <repository-name>
