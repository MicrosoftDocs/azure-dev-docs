...
The JavaScript/TypeScript Modernizer is available as part of the GitHub Copilot modernization extension for Visual Studio Code. Built on GitHub Copilot agent mode, this tool analyzes your project, suggests an upgrade plan, and automatically updates npm packages to their latest versions. It also helps you apply necessary code changes to accommodate breaking changes or new APIs.
...
- **Package updates**: Automatically updates npm packages to their latest versions by modifying `package.json` and running `npm install` or `npm update`.
- **Code remediation**: Helps apply code changes needed for breaking changes or new APIs introduced in updated packages.
...
To use the JavaScript/TypeScript Modernizer, you must enable an experimental setting in your Visual Studio Code configuration. The tool is currently optimized for single-project scenarios:
...
- **Experimental feature**: Because the JavaScript/TypeScript upgrade capability is enabled by an experimental setting and is under active development, it may have rough edges. Always review changes carefully before committing.
