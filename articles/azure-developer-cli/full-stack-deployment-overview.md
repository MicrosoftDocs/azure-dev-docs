...
**Sequential deployment**: For complex scenarios, you might need to deploy services in a specific order. Use `azd` hooks to control deployment sequence, ensuring that prerequisite services are running before dependent services are deployed.
...
1. **Use environment variables**: Pass configuration between infrastructure and application layers
...
1. **Consider deployment order**: Some scenarios might require deploying services in a specific sequence
  ...
  - **Configure service order**: In your `azure.yaml` file, define services in the order you want them deployed. While `azd` deploys services in parallel by default, you can use hooks to enforce sequential deployment when needed.

  - **Customize workflow steps**: Override the default `azd up` workflow by defining a custom `workflows` property in your `azure.yaml` file. For example, you can change the default behavior to run provisioning before building your application source code:
  ...
  - **Separate provision and deploy**: Instead of using `azd up`, run `azd provision` and `azd deploy` as separate commands. This separation is useful when you need to verify infrastructure configuration before deploying application code, or when troubleshooting deployment issues. You can provision infrastructure once, then deploy and redeploy application code multiple times without reprovisioning.

  - **Customize with hooks**: Add pre and post hooks in your `azure.yaml` file to execute custom logic between provisioning and deployment phases. Use hooks to populate configuration files, validate environment state, or coordinate complex deployment sequences.
...
1. **Use Azure Verified Modules (AVM)**: Leverage AVM Bicep modules like [`container-app-upsert`](https://github.com/Azure/bicep-registry-modules/tree/main/avm/ptn/azd/container-app-upsert) for container apps. These patterns work seamlessly with `azd`'s two-phase workflow to resolve circular dependencies.
1. **Customize workflows when needed**: For simple deployments, use `azd up` with default settings. For complex scenarios with circular dependencies, customize the `workflows` property in your `azure.yaml` file to control the order of package, provision, and deploy steps.
