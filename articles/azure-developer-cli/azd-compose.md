...
Running the `azd infra synth` command exits you from the `azd` compose feature and simplified init process. Changes you make to the Bicep files generated by `azd infra synth` are not accounted for by `azd` compose. For example, after you run `azd infra synth`, if you edit the Bicep code and then run `azd infra synth`, the Bicep files you edited are overwritten by the re-generated code.
