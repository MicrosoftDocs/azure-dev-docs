...
|---|---|---|---|---|
|**Local Machine via CLI**|**Not** in a container, dependencies are manually installed by you, and the project is run on your local machine.|You control all dependencies. You may already have some of the dependencies installed. You don't need Docker installed.|You have to manually install all dependencies.| Yes |
|**Visual Studio & Visual Studio Code**| [Run and debug `azd` templates](/azure/developer/azure-developer-cli/debug?pivots=ide-vs-code) using [Visual Studio (preview)](https://visualstudio.microsoft.com/vs/preview/) or the [Visual Studio Code extension](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.azure-dev).  | You can work with `azd` using IDE tooling you may already be comfortable with. UI abstractions are available for certain tasks. | Requires installing extensions or enabling features that are limited to a specific IDE. | Yes (preview) |
