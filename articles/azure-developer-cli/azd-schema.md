...
| `docker` | N | Only applicable when `host` is `containerapp` (including Container App Jobs). Can't contain extra properties. | See the [custom Docker sample](#docker-options-sample). `path` _(string)_: Path to the Dockerfile. Default: `./Dockerfile`; `context` _(string)_: The docker build context. When specified, overrides default context. Default: `.`; `platform` _(string)_: The platform target. Default: `amd64`; `remoteBuild` _(boolean)_: Enables remote ACR builds. Default: `false` |
...
| `config` | N | _(object)_ Extra configuration options for the service. For JavaScript/TypeScript services (`js` or `ts`), supports `packageManager` _(string)_ to explicitly set the package manager. Valid values: `npm`, `pnpm`, `yarn`. If omitted, `azd` auto-detects the package manager from the `packageManager` field in `package.json`, lock files, or defaults to `npm`. | `config: { packageManager: pnpm }` |
