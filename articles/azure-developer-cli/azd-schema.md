...
| `host` | Y | _(string)_ Type of Azure resource used for service implementation. If omitted, App Service will be assumed. | `appservice`, `containerapp`, `function`, `staticwebapp`, `aks` (only for projects deployable via `kubectl apply -f`), `springapp` (with the alpha feature enabled) | 
