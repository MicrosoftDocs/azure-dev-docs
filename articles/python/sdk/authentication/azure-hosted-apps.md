...
## Enable managed identity in the Azure resource hosting the app
...
The Azure CLI commands used to enable managed identity for an Azure resource are of the form `az <command-group> identity --resource-group <resource-group-name> --name <resource-name>`. See the following commands for popular Azure services.
...
The [az role assignment create](/cli/azure/role/assignment) command assigns a role to a managed identity. For the assignee, use the `principalId` you copied in step 1.
...
When your code is running in Azure and managed identity is enabled on the Azure resource hosting your app, the [`DefaultAzureCredential`](/python/api/azure-identity/azure.identity.defaultazurecredential) determines the credentials to use in the following order:
...
1. If managed identity is enabled on the Azure resource, use the system-assigned managed identity.
...
This article uses the system-assigned managed identity for an Azure App Service web app. Using this approach, you don't need to configure a managed identity in the environment or pass it in as a parameter. The following steps show you how to use `DefaultAzureCredential`.
