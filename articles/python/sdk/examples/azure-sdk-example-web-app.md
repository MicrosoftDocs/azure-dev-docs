...
The Azure SDK for Python includes management libraries (namespaces beginning with `azure-mgmt`) that you can use to automate resource configuration and deployment. This automation is similar to what you can do by using the Azure portal, Azure CLI, or ARM templates. For examples, see [Quickstart: Deploy a Python (Django or Flask) web app to Azure App Service](/azure/app-service/quickstart-python).
...
In your local development environment, install the requirements by using the following code:
...
1. Visit [https://github.com/Azure-Samples/python-docs-hello-world](https://github.com/Azure-Samples/python-docs-hello-world) and fork the repository into your own GitHub account. By using a fork, you ensure that you have the necessary permissions to deploy the app to Azure.

    ![Forking the sample repository on GitHub](../../media/azure-sdk-example-web-app/fork-github-repository.png)

1. Next, create an environment variable named `REPO_URL` and set it to the URL of your forked repository. The example code in the next section requires this variable.
...
Create a Python file named **provision_deploy_web_app.py** and add the following code. The in-line comments explain what each part of the script does. Set the `REPO_URL` and `AZURE_SUBSCRIPTION_ID` environment variables as described in the previous step.
...
The script provisions all the necessary resources to host your web app and configures the deployment source to use your forked repository by using manual integration. By using manual integration, you need to manually trigger the web app to pull updates from the specified repository and branch.
...
To redeploy your code by using the Azure CLI, run the [az webapp deployment source sync](/cli/azure/webapp/deployment/source#az-webapp-deployment-source-sync) command:
...
You don't need to change the resource group name (`--resource-group` option) unless you changed it in the script.
...
You can also delete a resource group programmatically by using the [`ResourceManagementClient.resource_groups.begin_delete`](/python/api/azure-mgmt-resource/azure.mgmt.resource.resources.operations.resourcegroupsoperations#azure-mgmt-resource-resources-operations-resourcegroupsoperations-begin-delete) method.
