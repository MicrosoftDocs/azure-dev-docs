...
The code examples in this article demonstrate common operations with Managed Disks using the `azure-mgmt-compute` library. These examples aren't meant to be run as standalone scripts, but rather to be integrated into your own code. To learn how to create a `ComputeManagementClient` instance from `azure.mgmt.compute` in your script, see [Example - Create a virtual machine](azure-sdk-example-virtual-machines.md).
...
This example shows how to create a Managed Disk from a VHD file stored in Azure Blob Storage. This method is helpful when you want to reuse or move an existing virtual hard disk into Azure.
...
This example shows how to create a Managed Disk image from a VHD file stored in Azure Blob Storage. This method is useful when you want to make a reusable image that you can use to create new virtual machines.
...
This example shows how to create a new Managed Disk by copying an existing one. This method is helpful when you want to make a backup or use the same disk setup on another virtual machine.

:::code language="python" source="~/../python-sdk-docs-examples/managed_disk/disk_from_image.py":::

## Virtual machine with Managed Disks

You can create a virtual machine with an implicitly created Managed Disk based on a specific disk image, so you don't need to manually define all disk details.

You implicitly create a Managed Disk when you create a VM from an OS image in Azure. Azure automatically handles the storage account, so you don't need to specify `storage_profile.os_disk` or create a storage account manually.

:::code language="python" source="~/../python-sdk-docs-examples/managed_disk/vm_with_managed_disks.py":::

For a complete example that shows how to create a virtual machine by using the Azure management libraries for Python, see [Example - Create a virtual machine](azure-sdk-example-virtual-machines.md). This example demonstrates how to use the `storage_profile` parameter.
...
With Azure Managed Disks, you no longer need to manage storage accounts. As a result, the `storage_profile` for [Virtual Machine Scale Sets](/azure/virtual-machine-scale-sets/overview) can now match the one used for individual VM creation:
...
This example shows how to make an existing Managed Disk larger. This change is useful when you need more space for your data or applications.
...
This example shows how to change the storage type of a Managed Disk and make it larger. This change is helpful when you need more space or better performance for your data or applications.
...
### Create a snapshot of a managed disk that's currently attached to a virtual machine
