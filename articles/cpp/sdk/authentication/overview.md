...
You configure the appropriate authentication method for each environment, and `DefaultAzureCredential` automatically detects and uses that authentication method. The use of `DefaultAzureCredential` is preferred over manually coding conditional logic or feature flags to use different authentication methods in different environments. For more information, see [How to customize DefaultAzureCredential](./credential-chains.md#how-to-customize-defaultazurecredential).
